    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-400">
          Review possible warning signs in the details you provide. This does
          not verify a website or prove legitimacy.
        </p>
      </div>
      ...
      {!customMode ? (
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <select
              ...
              onChange={(e) =>
                setCustomForm((f) => ({
                  ...f,
                  funding_type: e.target.value as FundingType
                }))
              }
              className="rounded-lg border border-dark-border bg-dark-card px-4 py-2.5 text-sm text-ink outline-none focus:border-accent/50"
            >
          ...
          <input
            ...
            onChange={(e) =>
              setCustomForm((f) => ({ ...f, location: e.target.value }))
            }
            className="w-full rounded-lg border border-dark-border bg-dark-card px-4 py-2.5 text-sm text-ink placeholder-gray-500 outline-none focus:border-accent/50"
          />
          <textarea
            aria-label="Notes or anything suspicious you noticed..."
      ...
      >
        {loading ? 'Analyzing...' : 'Review risk signals'}
      </button>
          ...
          <div className="rounded-xl border border-dark-border bg-dark-card p-6 backdrop-blur-sm">
            <div className="flex items-center justify-between">
              <h4 className="font-mono text-lg font-semibold text-ink">
                Unverified risk assessment
              </h4>
              <span
                className={`rounded-full px-4 py-1.5 text-lg font-bold ${getScoreColor(result.scam_score)}`}
              >
                AI opinion
              </span>
            </div>
